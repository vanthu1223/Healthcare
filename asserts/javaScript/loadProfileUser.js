...
async function loadUserProfile() {
  try {
    ...
    if (user_token.role == "user") {
      ...
      document.querySelector("#avatar").src ="https://haycafe.vn/wp-content/uploads/2022/02/Avatar-trang.jpg"
...
async function myFix() {
  ...
  try {
    ...
    document.querySelector('#username').innerHTML =  users.username;
    document.querySelector('#email').innerHTML =  users.email;
...
async function myPassword() {
  ...
  const isPasswordMatch = await checkPassword(oldPassword);
...
async function checkPassword(password) {
  try {
    ...
    if (response.ok) {
      ...
      return result.isMatch; // Trả về true nếu mật khẩu khớp, ngược lại trả về false
    } else {
